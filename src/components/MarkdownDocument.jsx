...
const markdownComponents = {
  ...
  ul: ({ children }) => (
    <ul className="mb-6 space-y-3 ml-2">{children}</ul>
  ),
  ol: ({ children }) => (
  ...
  a: ({ href, children }) => {
    const linkClass =
      "text-[#c5629a] hover:text-[#f390d0] transition-colors underline decoration-2 underline-offset-4 font-bold uppercase text-xs";
...
export default function MarkdownDocument({
  ...
  if (loading) {
    return <CascadeLoader label={loadingLabel} className="min-h-[50vh]" />;
  }

  const showHeader = headerTitle || headerEyebrow;
      ...
      {showHeader && (
        <BorderBox borderColor="bg-[#c5629a]" innerBg="bg-[#130217]">
          <div className="p-8 sm:p-12 text-center relative">
            {headerEyebrow && (
              <span className="text-[#c5629a] minecraftFont text-[10px] sm:text-xs uppercase tracking-[0.3em] mb-2 block">
                {headerEyebrow}
              </span>
            )}
            {headerTitle && (
              <h1 className="text-4xl sm:text-6xl font-bold text-white minecraftFont uppercase tracking-tighter leading-none flex items-center justify-center gap-4">
                {headerIcon && (
                  <i className={`hn ${headerIcon} text-[#c5629a] text-3xl sm:text-5xl`}></i>
                )}
                {headerTitle}
              </h1>
            )}
